...
- delta (post-review advisories): R2-dup-win-name resolved by extracting `dflowBinaryName()` (single platform-correct binary name for both call sites); R2-chmod-rationale resolved by dropping the redundant `os.Chmod` (WriteFile applies 0o755 at creation on a fresh t.TempDir() file) with the rationale in a comment. Verified: go vet clean, go test -count=1 ./cmd/tests/... ok. Committed as a follow-up work unit and re-reviewed as a delta lineage.
