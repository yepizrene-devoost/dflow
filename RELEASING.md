# Releasing dflow

This document describes the release flow currently used by this repository.

## Release Overview

`dflow` uses:

- `develop` as the main integration branch
- `main` as the release branch
- `release/*` branches to prepare release notes and final publication changes
- `GoReleaser` to build binaries and publish GitHub releases

The GitHub release body is taken from `CHANGELOG.md`.

Release credentials are loaded from a local `.env` file. Use `.env.example` as
the template and keep real tokens out of version control.

## Release Checklist

### 1. Finish the feature work

- Merge all work branches into `develop`
- Ensure `develop` is clean and up to date
- Run:

```bash
go test ./...
```

### 2. Create a release branch

Create a release branch from `develop`:

```bash
dflow start release v0.2.0
```

Adjust the version string to the release you are preparing.

### 3. Prepare release notes

On the release branch:

- Run `make changelog` to generate `CHANGELOG.draft.md` from the Conventional
  Commits since the last tag (requires `git-cliff`)
- Curate the draft into `CHANGELOG.md` under a section for the version being
  released: prune the internal noise, reword entries for users, and keep the
  human-readable voice — the changelog is generated **and** curated
- Update `HISTORY.md` with the new version, tag, and date
- Keep release-only documentation changes isolated in this branch

`CHANGELOG.draft.md` is a local helper: it is gitignored, never committed, and
safe to delete once the curation is done.

Run tests again:

```bash
go test ./...
```

### 4. Finish the release branch

From the `release/*` branch, run:

```bash
dflow finish
```

With the current repository configuration:

- `develop` is an `auto` target
- `main` is a `manual` target

That means `dflow finish` will merge the release branch back into `develop`,
but you still need a PR from `release/*` into `main`.

### 5. Merge to main

- Open a PR from the release branch to `main`
- Merge it
- Check out `main`
- Pull the merged changes

At this point, `main` should contain the exact state you want to publish.

### 6. Create the release tag

Create the tag from the current `main` commit:

```bash
git tag -a v0.2.0 -m "v0.2.0"
git push origin v0.2.0
```

If you need to fix release metadata after tagging but before publishing:

```bash
git tag -d v0.2.0
git push origin :refs/tags/v0.2.0
```

Then recreate the tag on the corrected commit.

### Retagging or rerunning a release

If you already created or pushed a tag and later need to correct release
metadata before publishing the final release:

```bash
git tag -d v0.2.0
git push origin :refs/tags/v0.2.0
```

Then:

1. Commit the metadata fix on `main`
2. Recreate the same tag on the corrected commit
3. Push the tag again
4. Rerun `make release`

If the GitHub release entry already exists, GoReleaser is configured with
`release.mode: replace`, so rerunning the release updates the existing body.

### 7. Publish the release

Push `main` if needed, then run:

```bash
make release
```

The release target:

- runs only on a tagged commit: `VERSION` comes from `git describe --tags
  --exact-match`, so an untagged tree fails the release before anything is
  built instead of publishing under a `dev` marker
- requires a `.env` file with a non-empty `GITHUB_TOKEN`; a missing,
  unreadable, or token-less `.env` stops the release before GoReleaser builds
  any artifact
- extracts the latest version's section from `CHANGELOG.md` into
  `RELEASE_NOTES.md` (generated, gitignored)
- validates that extracted section before writing it: if `CHANGELOG.md` is
  missing, carries no `## 📦` version section, or the latest such section has no
  content below its heading, `make release-notes` fails with a pointed message
  and a non-zero exit
- checks that the extracted section's version matches the tag being released;
  a stale or mismatched `CHANGELOG.md` stops the release instead of overwriting
  the published body with the wrong notes
- runs `goreleaser release --clean --release-notes=RELEASE_NOTES.md`

GoReleaser is configured to:

- use config format `version: 2`
- replace the GitHub release body if the release already exists
- attach `LICENSE`, `README.md`, and `CHANGELOG.md` to the archives

## Notes

- Always tag from `main`, never from `develop` or `release/*`
- Keep the working tree clean before tagging or publishing
- `CHANGELOG.md` is the cumulative record of what changed; the GitHub release
  body is only its latest version's section
- `RELEASE_NOTES.md` is generated by `make release` from the latest
  `CHANGELOG.md` section, is gitignored, and is never committed
- the notes step is validated before the release body is written and fails with
  a non-zero exit when `CHANGELOG.md` has no publishable content; because
  `make release` depends on it, nothing is published from an empty or partial
  release body
- `make release` must run on a tagged commit: it fails before building anything
  when `HEAD` is not exactly on a tag (`VERSION=dev`), so check out the release
  commit and create its tag first
- `make release` requires `.env` with a non-empty `GITHUB_TOKEN` before anything
  is built; a missing, unreadable, or token-less `.env` fails the release early
  instead of failing at the GoReleaser publish step
- the version in the newest `CHANGELOG.md` `## 📦` section must match the tag
  being released; a mismatch fails the release, so a stale or mismatched
  changelog stops the publish instead of overwriting the already published body
- `CHANGELOG.draft.md` is generated by `make changelog` (git-cliff) from the
  Conventional Commits; it is a curation input, never committed
- `HISTORY.md` is the long-form project history
- If `.env` contains old or unused tokens, remove them and rotate any exposed credentials
